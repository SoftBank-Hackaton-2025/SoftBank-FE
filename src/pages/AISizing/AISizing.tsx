...
import { postRecommendation } from "../../api/recommend"; // <- 경로 확인
...
const AISizing: React.FC = () => {
  ...
  const {
    sizingOptions,
    setSizingOptions,
    setCompletedSteps /*, setGenerationResults*/,
  } = useExpedition();
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    // 2) /tf-start 스펙에 맞춰 survey payload 구성
    const survey = {
    ...
    try {
      // 3) 추천 요청 (request_id는 내부에서 localStorage에서 읽음)
      const data = await postRecommendation(survey);

      console.log("/tf-start 응답:", data);

      // (선택) 응답을 컨텍스트에 저장하고 싶다면 아래처럼 활용
      // if (data?.data?.terraform || data?.data?.cost) {
      //   setGenerationResults?.(transformToCloudOptions(data));
      // }
